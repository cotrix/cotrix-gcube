...
public class DefaultSessionProvider implements SessionProvider {
	...
	private static final Logger logger = LoggerFactory.getLogger(DefaultSessionProvider.class);
	...
	public PortalSession sessionFor(SessionToken token) {
		...
		try {
			...
			String address = retrievePortalUrl(token.scope(), token.origin());
		
			URL serviceUrl = new URL(address + SERVICE_PATH);
			
			String response = makeRequest(serviceUrl, token.id());
